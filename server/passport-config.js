...
function completeUser(obj, accessToken, refreshToken, expiresIn) {
    const result = obj || {};
    const tokens = {
    ...
    result[config.uaa.tokensProp] = tokens;
...
function configurePassportStrategy() {
    ...
    cfStrategy = new CloudFoundryStrategy({
        clientID: config.uaa.userClientId,
        clientSecret: config.uaa.userClientSecret,
        callbackURL: config.uaa.callbackURL,
        uaaURL: config.uaa.uri,
        // Set skipUserProfile to false if you need user info AND have the openid scope
        skipUserProfile: false
    }, function (accessToken, refreshToken, params, profile, done) {
        const user = completeUser(profile, accessToken, refreshToken, params.expires_in);
    ...
    return function (req, res, next) {
        ...
        function fail() {
            if (options && options.noRedirect) {
                ...
                req.session[config.uaa.callbackProp] = req.baseUrl + req.path;
                res.redirect(config.uaa.loginURL);
            }
        }
            ...
            cfStrategy._oauth2.getOAuthAccessToken(
                ...
                function (err, newAccessToken, newRefreshToken, params) {
                    if (newAccessToken && newRefreshToken) {
                        completeUser(req.user, newAccessToken, newRefreshToken, params.expires_in);
                        success();
                    } else {
                        fail();
                    }
        ...
        if (req.isAuthenticated()) {
            tokens = req.user[config.uaa.tokensProp];

            if (tokens.expires &&
                moment.unix(tokens.expires)
                    .isAfter(moment().add(config.uaa.refreshWindow, "seconds"))) {
                success();
            } else {
